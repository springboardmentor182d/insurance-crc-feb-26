...
from src.database.models.policies import Policy, PolicyStatus
...
def _count_rows_in_range(session, model, date_column, start_dt: datetime, end_dt: datetime) -> int:
    stmt = (
        select(func.count())
        .select_from(model)
...
def get_admin_stats_snapshot() -> dict[str, float | int]:
    ...
    with SessionLocal() as session:
        ...
        total_claims = int(session.execute(select(func.count()).select_from(Claim)).scalar_one())
        ...
        users_current = _count_rows_in_range(session, User, User.created_at, current_start, now)
        users_previous = _count_rows_in_range(session, User, User.created_at, previous_start, current_start)
        ...
        claims_current = _count_rows_in_range(session, Claim, Claim.submitted_at, current_start, now)
        claims_previous = _count_rows_in_range(session, Claim, Claim.submitted_at, previous_start, current_start)
