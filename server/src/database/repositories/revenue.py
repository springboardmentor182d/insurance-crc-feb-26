...
def get_revenue_snapshot() -> list[dict[str, float | str]]:
    ...
    with SessionLocal() as session:
        premium_rows = session.execute(
            select(Policy.created_at, Policy.premium_amount).where(Policy.created_at >= start_window)
        ).all()
    ...
    expenses_by_month: dict[tuple[int, int], Decimal] = defaultdict(lambda: Decimal("0"))
