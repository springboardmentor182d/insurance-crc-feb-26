...
const StatusPill = ({ text, variant = 'default' }) => {
  const variants = {
    ...
    pending: 'bg-yellow-100 text-yellow-700', // ✅ ADD THIS
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-8">
      <div className="mb-6 flex items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-1">
            {viewMode === "ACTIVE" ? "My Active Policies" : "Pending Policies"}
          </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        ...
        {/* ACTIVE */}
        <div onClick={() => setViewMode("ACTIVE")} className="cursor-pointer">
          <SummaryCard
        ...
        {/* PENDING */}
        <div onClick={() => setViewMode("PENDING")} className="cursor-pointer">
          <SummaryCard
      ...
      <div className="mt-2">
        {pendingPolicies.length > 0 && (
          <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 px-4 py-3 rounded-lg mb-4">
            {pendingPolicies.length} policy{pendingPolicies.length > 1 ? "ies are" : " is"} pending admin approval.
          </div>
        )}
        {viewMode === "ACTIVE" &&
        ...
        {viewMode === "ACTIVE" && activePolicies.length === 0 && (
          <div className="bg-white rounded-xl shadow-sm p-6 text-sm text-gray-600 mt-4">
            No active policies yet. Waiting for admin approval.
          </div>
        )}

        {viewMode === "PENDING" && pendingPolicies.length === 0 && (
          <div className="bg-white rounded-xl shadow-sm p-6 text-sm text-gray-600 mt-4">
            No pending policies.
          </div>
        )}
            ...
            <form onSubmit={onExternalSubmit} className="px-6 py-4 space-y-6">
              ...
              <div>
                ...
                <div
                  ...
                  onDrop={onDrop}
                  className={`border-2 border-dashed rounded-xl px-4 py-6 text-center transition-colors ${isDraggingFiles ? 'border-blue-500 bg-blue-50' : 'border-gray-300 bg-gray-50'
                    }`}
                >
